...
export default {
  ...
  head: {
    ...
    meta: [
      ...
      { name: 'viewport', content: 'width=device-width, initial-scale=1' },
      {
        ...
        name: 'description',
        content:
          '都内のWebエンジニア、ハル(@hal_sea_)のブログ。主にプログラミングについて発信します。'
      },
  ...
  plugins: [
    ...
    { src: '@/plugins/unicon', mode: 'client' }
  ],
  ...
  generate: {
    async routes() {
      ...
      const pages = await axios
        .get(
          `https://${process.env.SERVICE_DOMAIN}.microcms.io/api/v1/blog?limit=0`,
          {
      ...
      const categories = await axios
        .get(
          `https://${process.env.SERVICE_DOMAIN}.microcms.io/api/v1/categories?fields=id`,
          {
            headers: { 'X-API-KEY': process.env.API_KEY }
          }
        )
      ...
      const tags = await axios
        .get(
          `https://${process.env.SERVICE_DOMAIN}.microcms.io/api/v1/tags?fields=id`,
          {
            headers: { 'X-API-KEY': process.env.API_KEY }
          }
        )
